...
function GalleryScene({
	...
	const spatialPositions = useMemo(() => {
		...
		for (let i = 0; i < visibleCount; i++) {
			...
			const x =
				...
				3;
			const y =
				(Math.cos(verticalAngle) * verticalRadius * maxVerticalOffset) / 4 + 2.0; // Added +2.0 to move images up
	...
	useFrame((state, delta) => {
		// Apply auto-play
		if (autoPlay) {
			setScrollVelocity((prev) => prev + 0.15 * delta); // Slowed from 0.3
		}
...
function FallbackGallery({ images }: { images: ImageItem[] }) {
	...
	return (
		<div className="flex flex-col items-center justify-center h-full bg-[#0A0A0C] p-4 text-white">
